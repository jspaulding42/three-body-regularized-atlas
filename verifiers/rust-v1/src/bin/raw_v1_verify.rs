...
use three_body_planar_chain_verifier::{
    execute_raw_v1_bytes_exact_rational_v04, RawV1EvaluationOutcome,
    RawV1ExecutionSerializationError, DEFAULT_WIRE_JSON_LIMITS,
};
...
fn run(arguments: impl IntoIterator<Item = OsString>) -> Result<u8, CliError> {
    ...
    let execution = execute_raw_v1_bytes_exact_rational_v04(&bytes);
    let json = execution
        .to_portable_json_bytes()
        .map_err(CliError::Serialization)?;
    let exit_code = if execution.evaluation_outcome() == RawV1EvaluationOutcome::Error {
...
enum CliError {
    ...
    Serialization(RawV1ExecutionSerializationError),
}
...
impl fmt::Display for CliError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Usage => formatter.write_str("usage: raw_v1_verify <raw-v1-path>"),
            Self::InputLimitOverflow => {
